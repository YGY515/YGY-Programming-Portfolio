...
- 원본 게임의 RPG Maker 엔진에서 기본적으로 제공되었던 기능을 직접 구현해 보며,<br>
  추후 기술 표준이 높아졌을 때 어떻게 <b>아키텍처</b>를 구성해야 하는지 고민하는 경험을 할 수 있었음.

- 구현 시 생겼던 오류를 Unity의 생명주기로 해결하며, <b>Unity 엔진의 생명주기</b>와 그에 맞춘 설계의 중요성을 배우게 되었음.

- 코드를 작성할 때 <b>‘왜 이렇게 해야 하는가?’</b> 라는 질문에 초점을 맞춰 작성하는 습관을 기를 수 있었음.

- 기능 구현 시 <b>확장성</b>과 <b>변경 가능성</b>을 우선 고려하여 코드를 작성하는 경험을 할 수 있었음.
...
- 근거리/원거리 두 무기를 사용하기에, <b>tab키</b>로 빠르게 무기를 <b>스와핑</b> 하도록 함.
  
- 각 무기마다 일정 확률로 치명타가 적중되며, 이때 플레이어의 카메라에 <b>줌 기능</b>을 호출함.
...
### 1) 페이즈에 따라 변하지 않은 NPC의 대사

**<문제 상황>**

보스의 페이즈에 따라 NPC 대사에 변화가 일어나지만,<br>
2페이즈임에도 불구하고 NPC의 대사는 <b>1페이즈</b>의 대사를 출력했음.<br>
...
- 보스의 페이즈에 급격한 변화가 있어도 누락 없이 안정적으로 처리되었음.
  
- 기술적으로 어려움을 겪은 문제는 <b>접근한 방식</b> 자체가 잘못됐는지를 먼저 점검하는 습관을 기르게 됨.

<br><br>

### 2) 번거로웠던 보스 페이즈 변수 참조

**<문제 상황>**

보스의 페이즈 번호를 <b>명확히</b> 알 수가 없었음.<br>
게임 내 여러 기능이 페이즈를 참조하지만, 정확한 페이즈 번호를 몰라 디버그에 어려움이 있었음. 

<br>

**<원인 분석>**

- 페이즈 변수를 <b>public int</b>로 관리했음. 

- 변수 참조 시 빈번하게 <b>Gameobject.find()</b>를 하거나 <b>수동</b>으로 인스펙터에 할당했음.

<br>

**<해결 방법>**
- 보스의 페이즈를 관리하는 매니저 스크립트에서 현재 페이즈 변수를 <b>인스턴스화</b> 함. ([해당 코드](https://github.com/YGY515/Game-Client-Programming-Portfolio/blob/main/%ED%94%84%EB%A1%9C%EC%A0%9D%ED%8A%B8%20%EC%83%98%ED%94%8C%20%EC%BD%94%EB%93%9C/%EC%BD%94%EB%93%9C%3A%20%EC%95%88%ED%8B%B0%EB%8D%B0%EC%8B%9C%EC%8A%A4%20%EB%A6%AC%EB%B9%8C%EB%93%9C/Manager/BossPhaseManager.cs))

- AdvanPhase() 함수를 호출하면 해당 변수가 증가하도록 함.

<br>

**<결과>**

- 모든 스크립트가 일관되게 보스 페이즈에 접근이 가능하게 됨.
...
작년 5월부터 12월까지 약 <b>8개월</b> 동안 제작했으며, 현재 <b>스토브</b>와 <b>스팀</b>에 데모 출시하였습니다.<br>

매주 <b>개발 일지</b>를 작성해 보며 제작 진도를 나간 것과 발생한 오류에 대한 대처를 기록하여 프로젝트를 되돌아보았습니다.<br>
...
- 피드백을 반영하기 전에 해당 내용이 게임 내 콘텐츠인지, 시스템 기능인지, 혹은 밸런싱인지 분류하며<br>
  우선수위를 두는 것이 중요함을 깨달았음.
...
- Suno와 같은 <b>생성형 AI</b>를 활용해 게임의 분위기에 부합하는 음악을 제작함. - [게임에 사용된 타이틀곡](https://suno.com/song/87b6e44f-818f-4538-bf89-0e2c3d45924f) <br>
  (해당 곡을 생성하기 위해 Dramatic and grand start, violin and cello, fast beat,
...
<b>'Cannot read property 'maps' of undefined'</b> 오류가 뜨며 게임 진행이 불가한 오류가 발생함.<br>
...
- 다른 플레이 기능에는 영향을 미치지 않으며, 오로지 <b>지도 사용 시</b> 오류가 발생했음.

- AI를 통해 코드를 분석한 결과, 해당 코드가 travelMaps라는 객체와 그 하위 객체인 maps가 <b>생성되기 전에 참조해서</b> 생긴 오류인 것으로 파악함.

<br><br>

**<해결 방법>**
<img align='right' img width="40%" src="https://github.com/user-attachments/assets/32ce950d-c6d8-4bd6-98b2-01ff0044be67" />
- 하위 객체를 참조하기 전에 <b>객체 초기화</b>를 하는 코드를 추가하여 해결함.<br><br>
...
테스터의 화면에서는 <b>다르게 보여</b> 몰입감이 깨지는 현상이 있었음.

<br>

**<원인 분석>**

- 그림자를 제거하거나 생성 시에 플레이어의 스프라이트 그림자를 <b>참조</b>하지만, 기기에 따라 정확하게 참조하지 않았음.

<br>

**<해결 방법>**
- <b>투명한 그림자 파일</b>을 새로 추가하고, 해당 파일로 <b>교체</b>하는 재사용 가능한 방식으로 해결함.
...
게임 내 <b>모든 글씨</b>가 때때로 <b>깨지는</b> 현상이 있었음.

<br>

**<원인 분석>**

- 모든 기기에서 확인되는 것이 아닌, <b>QHD(2560x1440) 이상의 모니터 해상도</b>에서 발생하는 것을 파악함.
...
- <b>UI 크기</b>가 화면의 해상도와 맞지 않아 생기는 원인으로 추정함.
...
- <b>스토리 컷씬</b>: Unity의 시네머신 카메라 기능을 활용하여, 대사 더빙과 하단의 자막을 통해 스토리 시청이 가능합니다.
...
일정이 촉박한 상황에서도 업무를 체계적으로 관리하여 최종적으로 앱 출시까지 마무리할 수 있었음.<br>
...
### 2) MAC 환경에서의 그래픽 오류 문제

**<문제 상황>**

iOS 빌드를 위해 맥북에 프로젝트를 import 했을 시 모든 오브젝트가 <b>핑크</b>로 보이는 그래픽 오류가 발생함.<br>

<br>

**<원인 분석>**

- Unity 버전과 그래픽 파이프라인의 <b>호환성</b> 문제로 추정함. <br>(MAC/iOS 환경에서 기본적으로 사용되는 Metal API와 프로젝트가 사용하던 렌더링 설정 간의 충돌 가능성)

<br>

**<해결 방법>**

- 프로젝트의 그래픽스에서 설정된 파이프라인을 확인하고 파이프라인 재설정 하였으나 해결되지 않았음.

- MAC 환경의 Unity 버전을 <b>상향</b>한 후 (2021.3.21f1 → 2022.3.3f1) 그래픽 에셋을 하나씩 import하여 해결함.
